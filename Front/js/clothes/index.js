...
function getPageClothes(currentPage, pageSize) {
    $('#core-clothes').empty();
    $.ajax({
        async: true,
        type: "GET",
        ...
        success: function (result) {
            if (result.head.code === '200') {
                // 说明有数据
                let domId = document.getElementById("core-clothes");
...
function spanSquare(domId, result, currentPage, pageSize) {
    ...
    pageTurningSquare[0].onclick = function () {
        if (currentPage === 1) {
            ...
            getPageClothes(currentPage - 1, pageSize);
        }
    };
