...
function getPageFood(currentPage, pageSize) {
    $('#core-food').empty();
    $.ajax({
        async: true,
        type: "GET",
        ...
        url: "http://localhost:8081/getPageIndexFood",
        success: function (result) {
            if (result.head.code === '200') {
                // 说明有数据
                let domId = document.getElementById("core-food");
                console.log(result);
...
function spanSquare(domId, result, currentPage, pageSize) {
    for (let i = 0; i < result.body.items.length; i++) {
        $(domId).append(`
            <div class="addedFood">
            <a href="../../html/food/food.html?id=${result.body.items[i].id}&category=${result.body.items[i].category}">
    ...
    pageTurningSquare[0].onclick = function () {
        if (currentPage === 1) {
            ...
            getPageFood(currentPage - 1, pageSize);
        }
    };
