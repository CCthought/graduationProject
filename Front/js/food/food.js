...
function getUniqueFood(id, category) {
    $.ajax({
        async: true,
        type: "GET",
        dataType: "json",
        ...
        url: "http://localhost:8081/getUniqueFood",
        success: function (result) {
            if (result.head.code === '200') {
                $('#message-picture').append(`<img src="../../images/${result.body.imgPath}" width="100%" height="100%" alt="picture" />`);
                document.getElementById('message-food-name').innerHTML = result.body.name;
                document.getElementById('message-food-price-li-del').innerHTML = result.body.price;
                document.getElementById('message-food-price-li-span').innerHTML = result.body.discount;
                ...
                document.getElementById('message-food-taste-description').innerHTML = result.body.description;
                // 购买按钮 和 购物车 注册鼠标移动事件
                $('.message-food-buyAndCart')[0].onmouseover = function () {
                    this.style.cursor = 'pointer';
                };
                $('.message-food-buyAndCart')[1].onmouseover = function () {
                    $('.message-food-buyAndCart')[1].style.cursor = 'pointer';
                };
                ...
                $('.message-food-buyAndCart')[1].onclick = function () {
