...
function getUniqueTrousers(id, category) {
    $.ajax({
        async: true,
        type: "GET",
        dataType: "json",
        ...
        url: "http://localhost:8081/getUniqueTrousers",
        success: function (result) {
            if (result.head.code === '200') {
                $('#message-picture').append(`<img src="../../images/${result.body.imgPath}" width="100%" height="100%" alt="picture" />`);
                document.getElementById('message-trousers-name').innerHTML = result.body.name;
                document.getElementById('message-trousers-price-li-del').innerHTML = result.body.price;
                document.getElementById('message-trousers-price-li-span').innerHTML = result.body.discount;
                ...
                let colorUl = document.getElementById('message-trousers-color-ul');
                for (let i = 0; i < result.body.color.length; i++) {
                    $(colorUl).append(` <li class="message-trousers-color-li">${result.body.color[i]}</li>`);
                }
                let sizeUl = document.getElementById('message-trousers-size-ul');
                for (let i = 0; i < result.body.size.length; i++) {
                    $(sizeUl).append(` <li class="message-trousers-size-li">${result.body.size[i]}</li>`);
                }
                ...
                // 为颜色 注册点击事件
                let colorLi = document.getElementById('message-trousers-color-ul').children;
                ...
                //为size 注册点击事件
                let sizeLi = document.getElementById('message-trousers-size-ul').children;
                ...
                // 购买按钮 和 购物车 注册鼠标移动事件
                $('.message-trousers-buyAndCart')[0].onmouseover = function () {
                    this.style.cursor = 'pointer';
                };
                $('.message-trousers-buyAndCart')[1].onmouseover = function () {
                    $('.message-trousers-buyAndCart')[1].style.cursor = 'pointer';
                }
                ...
                $('.message-trousers-buyAndCart')[1].onclick = function () {
                    let colorFlag = false;
                    let sizeFlag = false;
                    for (let i = 1; i < colorLi.length; i++) {
