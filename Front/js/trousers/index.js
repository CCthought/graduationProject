...
function getPageTrousers(currentPage, pageSize) {
    $('#core-trousers').empty();
    $.ajax({
        async: true,
        type: "GET",
        ...
        url: "http://localhost:8081/getPageIndexTrousers",
        success: function (result) {
            if (result.head.code === '200') {
                // 说明有数据
                let domId = document.getElementById("core-trousers");
...
function spanSquare(domId, result, currentPage, pageSize) {
    for (let i = 0; i < result.body.items.length; i++) {
        $(domId).append(`<div class="addedTrousers"><a href="../../html/trousers/trousers.html?id=${result.body.items[i].id}&category=${result.body.items[i].category}"><img src="../../images/${result.body.items[i].imgPath}" width="100%" height="260px" alt="picture"/></a><span style="display: inline-block;font-size: 16px;">${result.body.items[i].name}<br />原价￥${result.body.items[i].price} 折扣价<span style="color: red;">￥${result.body.items[i].discount}</span><br />销售量:${result.body.items[i].saled}</span></div>`);
    }
    // 增加<  >小方块
    ...
    pageTurningSquare[0].onclick = function () {
        if (currentPage === 1) {
            ...
            getPageTrousers(currentPage - 1, pageSize);
        }
    };
